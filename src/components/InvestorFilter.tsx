...
export default function InvestorFilter({ options, selected, onChange, metric }: Props) {
  ...
  const [width, setWidth] = useState(320) // = w-80
  ...
  // Log scale: money spans 0–~19.000 MM, so a linear bar would be unreadable.
  ...
  // Drag the right edge to resize. Listeners on window so the drag survives the
  // cursor leaving the thin handle.
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 border-b border-gray-200 px-3 py-2">
        <h3 className="text-sm font-semibold">{t('sankey.investors')}</h3>
        {selected.length > 0 && (
          <button onClick={() => onChange([])} className="text-xs text-gray-500 underline hover:text-gray-900">
            {t('common.clear')} ({selected.length})
          </button>
        )}
      </div>
      <div className="border-b border-gray-200 px-3 py-2">
        ...
        <div className="mt-1 flex items-center justify-between gap-2">
          ...
          <div className="flex shrink-0 overflow-hidden rounded border border-gray-300 text-[11px]">
      ...
      <div
        ...
        className="absolute right-0 top-0 h-full w-1.5 cursor-col-resize hover:bg-teal-400/40"
        aria-hidden
      />
    </aside>
  )
